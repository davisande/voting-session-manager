...
public class CreateTopicUseCaseTest {
    private static final String ILLEGAL_ARGUMENT_MESSAGE = "topicDataTransfer is marked non-null but is null";
    ...
    void testCreateTopicWhenHasMandatoryCreationDataExpectTopicSaved() {
        ...
        final var creationTopicDataTransfer = TopicDataTransferMockBuilder.buildCreationMock(TOPIC_DESCRIPTION);

        Mockito.when(topicMapper.topicDataTransferToTopic(Mockito.any())).thenReturn(creationTopic);
    ...
    void testCreateTopicWhenCreationObjectIsNullExpectedNullPointerError() {
        var illegalArgumentException = Assertions.catchNullPointerException(() -> createTopicUseCase.createTopic(null));

        Assertions.assertThat(illegalArgumentException).hasMessageContaining(ILLEGAL_ARGUMENT_MESSAGE);
    }
    ...
    void testCreateTopicWhenDatabaseErrorHappensExpectedUnexpectedError() {
        ...
        Mockito.lenient().when(topicMapper.topicDataTransferToTopic(Mockito.any())).thenReturn(creationTopic);
